...
function top(){
        ...
        echo -e "$red[~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~]"
        echo -e "$red[                                                      ]"
  echo -e "$red[  $red    WI SNIPER $version" "${yellow} ${red}  < WI""${yellow}SNIPER" "${red}I""${yellow}s" "${red}T""${yellow}he ""${red}F""${yellow}uture >     "              ${blue}"    ]"
        echo -e "$blue[                                                      ]"
...
if [ $FLUX_DEBUG != 1 ]; then
        ...
        sleep 0.1
        echo -e $red"                     WI SNIPER "$white""$version" (rev. "$green "$revision"$white") "$yellow"Created by "$white"ATHEX H4CK3R"
        sleep 0.1
        echo -e $green "           Page:"$red"https://github.com/Athexhacker/WI-SNIPER  "$transparent
        sleep 0.1
        echo -n "                              Latest rev."
        tput civis
        checkupdatess &
...
if [ "$FLUX_AUTO" =  "1" ];then
        ...
        while true; do
                ...
                echo "                                       "
                echo -n -e ""$red"["$blue"deltaxSNIPER"$yellow"@"$white"WI SNIPER"$red"]-["$yellow"~"$red"]"$transparent""
                read yn
                echo ""
                case $yn in
        ...
        if [ "$INTERFACESNUMBER" -gt "0" ]; then
                ...
                if [ "$INTERFACESNUMBER" -eq "1" ]; then
                        ...
                        if [ "$FLUX_AUTO" = "1" ];then
                                ...
                                echo
                                echo -n -e ""$red"["$blue"deltaxflux"$yellow"@"$white"WI SNIPER"$red"]-["$yellow"~"$red"]"$transparent""
                                read line
                        fi
